...
function generateGroupContractHtml(orderId, kind) {
  return safeCall(function() {
    var grp = getGroupMembers_(orderId);
    var d   = getDocData_(orderId);
    var map = buildDocPlaceholders_(d, '');   // реквизиты компании и заказчика (юрлицо) — от родителя
    var isLegal = String(d.client['Тип'] || '') === 'юр' || String(d.order['_clientType'] || '') === 'юр';
    var byProxy = isLegal && String(d.order['Подписант'] || '').trim() !== '';

    var anyTint = grp.members.some(function(m){ return /тонир|tint/i.test(m.service); });
    var anyWrap = grp.members.some(function(m){ return /оклей|ppf|полир|керам|антихром|шумо|химч|бронир/i.test(m.service); });
    var svcWord = (anyTint && anyWrap) ? 'тонировке и оклейке' : (anyTint ? 'тонировке' : 'оклейке');

    var total = grp.total;
    map['Сумма цел']      = String(Math.round(total));
    map['Сумма прописью'] = numToWords_(total);

    var rows = grp.members.map(function(m, i){
      var pl  = String(m.film || '').trim() + (String(m.light || '').trim() ? ' ' + m.light + '%' : '');
      var svc = /тонир|tint/i.test(m.service) ? 'Тонировка' : (/оклей|ppf/i.test(m.service) ? 'Оклейка' : (m.service || ''));
      return '<tr><td class="center">' + (i + 1) + '</td><td>' + (m.car || '') + '</td><td>' + (m.vin || '') +
        '</td><td>' + svc + '</td><td>' + (pl || '—') + '</td><td class="right">' + (Number(m.price) || 0).toFixed(2) + '</td></tr>';
    }).join('');
    var vehTable = '<table><thead><tr><th style="width:28px">№</th><th>Марка, модель</th><th>VIN</th>' +
      '<th style="width:90px">Услуга</th><th>Плёнка / светопроп.</th><th style="width:96px">Стоимость, руб.</th></tr></thead>' +
      '<tbody>' + rows + '<tr><td colspan="5" class="right"><b>Итого:</b></td><td class="right"><b>' + total.toFixed(2) + '</b></td></tr></tbody></table>';

    var body  = fillTemplate_(groupDocTemplate_(kind, isLegal, byProxy, svcWord, vehTable), map);
    var title = (kind === 'act' ? 'Акт ' : 'Договор ') + (map['Номер договора'] || '') + ' — ' + grp.count + ' авто';
    return docWrap_(title, body, map['Логотип'], true);
  });
}

/** Шаблон группового договора/акта (несколько авто). */
function groupDocTemplate_(kind, isLegal, byProxy, svcWord, vehTable) {
  ...
  var clientIntro = isLegal
    ? '<b>{{Компания заказчика}}</b>, именуемое в дальнейшем «Заказчик», ' + repClause
    : '<b>{{ФИО}}</b>, именуемый в дальнейшем «Заказчик»';
  var clientReq = isLegal
    ? '{{Компания заказчика}}<br>УНП: {{УНП заказчика}}<br>{{Банк заказчика}}<br>Юр. адрес: {{Адрес заказчика}}<br><br>' + signRole + ' _____________ ' + signName + '<br>М.П.'
    : 'ФИО: {{ФИО}}<br>Адрес: {{Адрес заказчика физ}}<br>Паспорт: {{Паспорт}}<br><br>_____________ / {{Фамилия И.О.}}';

  if (kind === 'act') {
    return '<h2>АКТ сдачи-приёмки выполненных работ</h2>' +
      '<div class="bar"><span>г. Минск</span><span>{{Дата ru}}</span></div>' +
      '<p>к договору № {{Номер договора}} от {{Дата ru}}.</p>' +
      '<p>{{Компания}}, именуемое «Исполнитель», в лице директора {{Директор род.}}, с одной стороны, и ' + clientIntro +
      ', с другой стороны, составили настоящий акт о том, что Исполнитель выполнил, а Заказчик принял работы по ' + svcWord + ' следующих автомобилей:</p>' +
      vehTable +
      '<p>Работы выполнены в полном объёме и в срок. Стороны претензий друг к другу не имеют. Общая стоимость выполненных работ составляет <b>{{Сумма цел}} бел. рублей 00 коп.</b> ({{Сумма прописью}}).</p>' +
      '<table style="margin-top:14px"><tr>' +
      '<td style="width:50%"><b>Исполнитель:</b><br>{{Компания}}<br>УНП: {{УНП}}<br><br>Директор _____________ {{Директор}}<br>М.П.</td>' +
      '<td style="width:50%"><b>Заказчик:</b><br>' + clientReq + '</td></tr></table>';
  }

  return '<h2>ДОГОВОР № {{Номер договора}}</h2>' +
    '<div class="bar"><span>г. Минск</span><span>{{Дата ru}}</span></div>' +
    '<p>{{Компания}}, именуемое в дальнейшем «Исполнитель», в лице директора {{Директор род.}}, действующего на основании Устава, с одной стороны, и ' + clientIntro + ', заключили настоящий договор о нижеследующем:</p>' +
    '<h3>1. Предмет договора</h3>' +
    '<p>1.1. Исполнитель обязуется выполнить работы по ' + svcWord + ' автомобилей Заказчика согласно перечню (п.1.2), а Заказчик обязуется принять результат выполненных работ и оплатить их стоимость.</p>' +
    '<p>1.2. Перечень автомобилей и работ:</p>' +
    vehTable +
    '<p>1.3. Сроки выполнения работ: начало — {{Дата начала кратко}}, окончание — {{Дата окончания кратко}}.</p>' +
    '<h3>2. Цена договора и порядок расчётов</h3>' +
    '<p>2.1. Общая стоимость работ по настоящему договору составляет <b>{{Сумма цел}} бел. рублей 00 коп.</b> ({{Сумма прописью}}).</p>' +
    '<p>2.2. Стоимость по каждому автомобилю указана в перечне (п.1.2). Окончательная стоимость отражается в Акте сдачи-приёмки выполненных работ.</p>' +
    '<p>2.3. Оплата производится в порядке, согласованном сторонами, в соответствии с законодательством Республики Беларусь. За нарушение срока оплаты Заказчик уплачивает пеню в размере 0,1% от неоплаченной в срок суммы за каждый день просрочки, но не более суммы задолженности.</p>' +
    '<h3>3. Права и обязанности сторон</h3>' +
    '<p>3.1. Исполнитель обязуется выполнить работы с надлежащим качеством и в согласованные сроки, используя собственные материалы, и сдать результат по Акту сдачи-приёмки.</p>' +
    '<p>3.2. Заказчик обязуется предоставить автомобили, оплатить работы и принять их результат. Если Заказчик в течение 3 (трёх) рабочих дней с момента уведомления о готовности не подписал Акт и не заявил мотивированных письменных возражений, работы считаются принятыми в полном объёме.</p>' +
    '<h3>4. Ответственность сторон</h3>' +
    '<p>4.1. За ненадлежащее исполнение обязательств стороны несут ответственность в соответствии с законодательством Республики Беларусь.</p>' +
    '<p>4.2. Ответственность Исполнителя за качество работ ограничивается стоимостью соответствующих работ. Исполнитель не отвечает за скрытые дефекты лакокрасочного покрытия и стёкол, существовавшие до начала работ.</p>' +
    '<h3>5. Гарантийные обязательства</h3>' +
    '<p>5.1. Срок гарантии на выполненные работы составляет {{Гарантия мес}} месяцев с даты подписания Акта при соблюдении правил эксплуатации. Гарантия на плёночное покрытие — 10 лет.</p>' +
    '<p>5.2. Правила эксплуатации: не мыть автомобиль в течение 10 дней после установки; по истечении 10 дней явиться на контрольный осмотр; в дальнейшем — ручная мойка неагрессивными составами, не подносить сопло мойки высокого давления ближе 50 см к краям плёнки; не использовать автоматические (щёточные) мойки.</p>' +
    '<p>5.3. Гарантия не распространяется на недостатки вследствие: механических повреждений, сколов от камней и гравия; ДТП; воздействия агрессивных химических веществ и реагентов; мойки в первые 10 дней и мойки на автоматических мойках; нарушения правил эксплуатации; самостоятельного вмешательства в покрытие; естественного износа; а также на дефекты ЛКП и стёкол, имевшиеся до выполнения работ.</p>' +
    '<h3>6. Обстоятельства непреодолимой силы</h3>' +
    '<p>6.1. Стороны освобождаются от ответственности за неисполнение обязательств, вызванное обстоятельствами непреодолимой силы (форс-мажор). Срок исполнения продлевается соразмерно времени их действия.</p>' +
    '<h3>7. Порядок разрешения споров</h3>' +
    '<p>7.1. Споры разрешаются путём переговоров, а при недостижении согласия — в судебном порядке в соответствии с законодательством Республики Беларусь.</p>' +
    '<h3>8. Срок действия договора. Изменение и расторжение</h3>' +
    '<p>8.1. Договор вступает в силу с момента подписания и действует до полного исполнения обязательств. Изменения и дополнения действительны в письменной форме за подписями сторон. Расторжение не освобождает Заказчика от оплаты фактически выполненных работ.</p>' +
    '<h3>9. Заключительные положения</h3>' +
    '<p>9.1. Договор составлен в двух экземплярах, имеющих одинаковую юридическую силу. Неотъемлемой частью договора является Акт сдачи-приёмки выполненных работ.</p>' +
    '<h3>10. Адреса, банковские реквизиты и подписи сторон</h3>' +
    '<table><tr>' +
    '<td style="width:50%"><b>Исполнитель:</b><br>{{Компания}}<br>Р/сч: {{Р/с}} в {{Банк}}, УНП: {{УНП}}<br>Юр. адрес: {{Юр.адрес}}<br>Тел.: {{Телефон}}<br><br>Директор _____________ {{Директор}}<br>М.П.</td>' +
    '<td style="width:50%"><b>Заказчик:</b><br>' + clientReq + '</td></tr></table>';
}
...
/**
 * АКТ приёма автомобиля В РАБОТУ (подписывается при сдаче авто до начала работ).
 * Фиксирует согласованный перечень элементов к оклейке — чтобы после работ не было
 * споров: если оклеены все заявленные элементы, работы выполнены в полном объёме.
 */
function generateAcceptanceActHtml(orderId, service) {
  return safeCall(function() {
    var d = getDocData_(orderId);
    var map = buildDocPlaceholders_(d, service);
    var isLegal = String(d.client['Тип'] || '') === 'юр' || String(d.order['_clientType'] || '') === 'юр';
    var byProxy = isLegal && String(d.order['Подписант'] || '').trim() !== '';
    var body = fillTemplate_(acceptanceActBodyHtml_(isLegal, byProxy), map);
    return docWrap_('Акт приёма авто ' + (map['Номер договора'] || ''), body, map['Логотип'], true);
  });
}

/** Шаблон акта приёма автомобиля в работу с согласованным перечнем элементов к оклейке. */
function acceptanceActBodyHtml_(isLegal, byProxy) {
  var repClause = byProxy
    ? 'в лице {{Подписант род.}}, действующего на основании доверенности {{Доверенность}}'
    : 'в лице директора {{Директор заказчика род.}}, действующего на основании Устава';
  var clientAct = isLegal
    ...
    : '<b>{{ФИО}}</b>, именуемый в дальнейшем «Заказчик»';
  var clientSign = isLegal ? (byProxy ? '{{Подписант}}' : '{{Директор заказчика}}') : '{{Фамилия И.О.}}';
  return '' +
    '<h2>АКТ приёма автомобиля в работу № {{Номер договора}}</h2>' +
    '<div class="bar"><span>г. Минск</span><span>{{Дата ru}}</span></div>' +
    '<p>{{Компания}} в лице директора {{Директор род.}}, именуемое в дальнейшем «Исполнитель», с одной стороны, и ' + clientAct + ', с другой стороны, составили настоящий акт о нижеследующем.</p>' +
    '<p>1. Заказчик передал, а Исполнитель принял в работу автомобиль {{Марка авто}} {{Модель}}, VIN {{VIN}}, гос. номер {{Гос.номер}}, для выполнения работ по оклейке защитной плёнкой {{Пленка}} (комплекс «{{Комплекс}}»).</p>' +
    '<p>2. Стороны согласовали перечень элементов автомобиля, подлежащих оклейке в рамках настоящего заказа:</p>' +
    '{{Таблица элементов оклейки}}' +
    '<p>3. Заказчик подтверждает, что ознакомлен и согласен именно с указанным в п.2 перечнем элементов. Работы выполняются в объёме перечисленных элементов; элементы, не указанные в перечне, в объём и стоимость работ не входят.</p>' +
    '<p>4. Стоимость согласованных работ составляет <b>{{Сумма цел}} руб. 00 коп.</b> ({{Сумма прописью}}).</p>' +
    '<p>5. После завершения работ автомобиль передаётся Заказчику. При наличии оклейки <b>всех</b> перечисленных в п.2 элементов работы считаются выполненными в полном объёме и надлежащего качества. Претензии по объёму работ в части элементов, не включённых в согласованный перечень (п.2), Исполнителем <b>не принимаются</b>.</p>' +
    '<p>6. Настоящий акт составлен в двух экземплярах, по одному для каждой стороны, и является неотъемлемой частью договора № {{Номер договора}}.</p>' +
    '<div class="sign" style="margin-top:18px"><span>Автомобиль в работу сдал (Заказчик):<br>_____________ / ' + clientSign + '<br><span class="muted">(подпись — с перечнем элементов согласен)</span></span>' +
    '<span>Автомобиль в работу принял (Исполнитель):<br>_____________ / {{Директор}}</span></div>';
}
...
function buildDocPlaceholders_(d, serviceMode) {
  ...
  return {
    ...
    // Нумерованная таблица элементов, СОГЛАСОВАННЫХ к оклейке (для акта приёма авто в работу)
    'Таблица элементов оклейки': (function(){
      var arr = String(o['Элементы'] || '').split(/\r?\n/).map(function(s){ return s.trim(); }).filter(Boolean);
      if (!arr.length) arr = ['Капот полностью','Передний бампер','Крылья целиком','Передняя оптика','Зеркала','Стойки лобового стекла','Полоса на крыше','Внутренние пороги','Кромки дверей','Антиманикюр'];
      var rows = arr.map(function(e, i){ return '<tr><td class="center">' + (i + 1) + '</td><td>' + e + '</td><td class="center">☐</td></tr>'; }).join('');
      return '<table><thead><tr><th style="width:34px">№</th><th>Элемент, подлежащий оклейке</th><th style="width:96px">Отметка</th></tr></thead><tbody>' + rows + '</tbody></table>';
    })(),
...
function contractTemplateHtml_(isLegal, byProxy) {
  ...
  <p>3.2.2. Принять результат выполненных работ и подписать Акт сдачи-приёмки. Если Заказчик в течение 3 (трёх) рабочих дней с момента уведомления о готовности не подписал Акт и не заявил мотивированных письменных возражений, работы считаются принятыми в полном объёме без замечаний.</p>
  ...
  <p>4.2. За нарушение срока оплаты Заказчик уплачивает Исполнителю пеню в размере 0,1% от неоплаченной в срок суммы за каждый день просрочки, но не более суммы задолженности.</p>
  <p>4.3. Ответственность Исполнителя за качество выполненных работ ограничивается стоимостью соответствующих работ по настоящему договору. Исполнитель не несёт ответственности за скрытые дефекты лакокрасочного покрытия и стёкол автомобиля, существовавшие до начала выполнения работ.</p>
  ...
  <p>5.6. Гарантия не распространяется на недостатки, возникшие вследствие: механических повреждений, сколов и повреждений от камней, гравия и иных дорожных факторов; дорожно-транспортных происшествий; воздействия агрессивных химических веществ, реагентов, битума, растворителей; мойки автомобиля в течение первых 10 дней после установки, а также мойки на автоматических (щёточных) мойках; нарушения Заказчиком правил эксплуатации (п.5.2–5.5); самостоятельного вмешательства Заказчика или третьих лиц в плёночное покрытие; естественного износа; а также на дефекты лакокрасочного покрытия и стёкол, имевшиеся до выполнения работ.</p>

  <h3>6. Обстоятельства непреодолимой силы</h3>
  <p>6.1. Стороны освобождаются от ответственности за частичное или полное неисполнение обязательств по настоящему договору, если оно явилось следствием обстоятельств непреодолимой силы (форс-мажор), возникших после заключения договора. Срок исполнения обязательств продлевается соразмерно времени действия таких обстоятельств.</p>

  <h3>7. Порядок разрешения споров</h3>
  <p>7.1. Все споры, возникающие из настоящего договора, разрешаются Сторонами в порядке переговоров. В случае невозможности разрешения споров в ходе переговоров, спор передаётся на рассмотрение в судебные инстанции в порядке, установленном действующим законодательством Республики Беларусь.</p>

  <h3>8. Срок действия договора. Изменение и расторжение</h3>
  <p>8.1. Настоящий договор вступает в силу с момента его подписания полномочными представителями обеих сторон и действует до полного исполнения сторонами принятых на себя обязательств.</p>
  <p>8.2. Все изменения и дополнения, вносимые в настоящий договор, действительны при условии, что они выполнены в письменной форме и подписаны полномочными представителями обеих сторон.</p>
  <p>8.3. Договор может быть расторгнут по соглашению сторон, а также в одностороннем порядке в случаях, предусмотренных законодательством Республики Беларусь. Расторжение договора не освобождает Заказчика от оплаты фактически выполненных Исполнителем работ и использованных материалов.</p>
  ...
  <p>9.2. Неотъемлемой частью настоящего договора являются: Протокол согласования цены; Акт сдачи-приёмки выполненных работ.</p>
