...
function generateContractHtml(orderId) {
  return safeCall(function() {
    ...
    var body = fillTemplate_(contractTemplateHtml_(isLegal), map);
 ...
 *  isLegal=true — Заказчик юрлицо (в лице директора, на основании Устава). */
function contractTemplateHtml_(isLegal) {
  // Сторона Заказчика во вводной части
  var clientIntro = isLegal
    ? `<b>{{Компания заказчика}}</b>, именуемое в дальнейшем «Заказчик», в лице директора {{Директор заказчика род.}}, действующего на основании Устава, с другой стороны, заключили настоящий договор о нижеследующем:`
  ...
  var clientSign = isLegal
    ? `<b>Заказчик:</b><br>{{Компания заказчика}}<br>УНП: {{УНП заказчика}}<br>{{Банк заказчика}}<br>Юр. адрес: {{Адрес заказчика}}<br><br>Директор _____________ {{Директор заказчика}}<br>М.П.`
    : `<b>Заказчик:</b><br>{{ФИО}}<br>Паспорт: {{Паспорт}}<br><br><br>_____________ / {{Фамилия И.О.}}`;
  // Заказчик в акте
  var clientAct = isLegal
    ? `<b>{{Компания заказчика}}</b> в лице директора {{Директор заказчика род.}}, именуемое в дальнейшем «Заказчик»`
    : `<b>{{ФИО}}</b>, именуемый в дальнейшем «Заказчик»`;
  var clientName = isLegal ? `{{Компания заказчика}}` : `{{ФИО}}`;
  var clientActSign = isLegal ? `{{Директор заказчика}}` : `{{Фамилия И.О.}}`;
  return `
