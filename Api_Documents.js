...
function buildDocPlaceholders_(d) {
  ...
  return {
    ...
    // Описание работ: тонировка — с названием плёнки и светопропускаемостью; оклейка — без названия
    'Описание работ':  (/тонир|tint/i.test(o['Услуга'] || '')
                          ? ('тонировочной плёнки' + (film ? ' ' + film : '') + (o['Светопропускаемость'] ? ' ' + o['Светопропускаемость'] + '%' : ''))
...
function contractTemplateHtml_(isLegal, byProxy) {
  ...
  var clientIntro = isLegal
    ? `<b>{{Компания заказчика}}</b>, именуемое в дальнейшем «Заказчик», ` + repClause + `, с другой стороны, заключили настоящий договор о нижеследующем:`
    : `<b>{{ФИО}}</b>, паспорт серии {{Паспорт}}, именуемый в дальнейшем «Заказчик», с другой стороны, заключили настоящий договор о нижеследующем:`;
  // Реквизиты/подпись Заказчика
  var clientSign = isLegal
    ? `<b>Заказчик:</b><br>{{Компания заказчика}}<br>УНП: {{УНП заказчика}}<br>{{Банк заказчика}}<br>Юр. адрес: {{Адрес заказчика}}<br><br>` + signRole + ` _____________ ` + signName + `<br>М.П.`
    : `<b>Заказчик:</b><br>{{ФИО}}<br>Паспорт: {{Паспорт}}<br><br><br>_____________ / {{Фамилия И.О.}}`;
  // Заказчик в акте
  ...
  <h3>1. Предмет договора</h3>
  <p>1.1. Исполнитель по настоящему договору обязуется выполнить работы по установке {{Описание работ}} на автомобиль {{Марка авто}} {{Модель}}, гос. номер {{Гос.номер}}, VIN {{VIN}}, а Заказчик обязуется принять результат выполненных работ и оплатить их стоимость.</p>
  ...
  <table><tr>
    ...
    <td style="width:50%">` + clientSign + `</td>
  </tr></table>
  ...
  <p>{{Компания}} в лице директора {{Директор род.}}, именуемое в дальнейшем «Исполнитель», с одной стороны, и ` + clientAct + `, с другой стороны, составили настоящий акт о том, что в соответствии с условиями договора № {{Номер договора}} от {{Дата}} г. Исполнитель выполнил следующую работу: установка {{Описание работ}} на автомобиль {{Марка авто}} {{Модель}}, VIN {{VIN}}.</p>
