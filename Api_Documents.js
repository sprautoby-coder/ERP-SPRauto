...
/**
 * АКТ приёма-передачи автомобиля с ОБЯЗАТЕЛЬНЫМ перечнем всех оклеенных элементов.
 * Клиент подписью подтверждает каждый элемент — защищает от претензий «что-то не доклеили».
 */
function generateAcceptanceActHtml(orderId, service) {
  return safeCall(function() {
    ...
    return docWrap_('Акт приёма ' + (map['Номер договора'] || ''), body, map['Логотип'], true);
  });
}

/** Шаблон акта приёма-передачи с перечнем оклеенных элементов и подписью заказчика. */
function acceptanceActBodyHtml_(isLegal, byProxy) {
  ...
  return '' +
    '<h2>АКТ приёма-передачи автомобиля и выполненных работ № {{Номер договора}}</h2>' +
    '<div class="bar"><span>г. Минск</span><span>{{Дата ru}}</span></div>' +
    ...
    '<p>1. Исполнитель выполнил, а Заказчик принял работы по оклейке защитной плёнкой {{Пленка}} автомобиля {{Марка авто}} {{Модель}}, VIN {{VIN}}, гос. номер {{Гос.номер}}. Комплекс работ: «{{Комплекс}}».</p>' +
    '<p>2. В рамках настоящего заказа оклеены перечисленные ниже элементы автомобиля. Заказчик принял каждый элемент по факту наличия и качества оклейки:</p>' +
    '{{Таблица элементов оклейки}}' +
    '<p>3. Стоимость выполненных работ составляет <b>{{Сумма цел}} руб. 00 коп.</b> ({{Сумма прописью}}).</p>' +
    '<p>4. Заказчик лично осмотрел автомобиль и результат работ, проверил наличие и качество оклейки <b>каждого</b> из перечисленных в п.2 элементов. Работы приняты <b>в полном объёме</b>; претензий по перечню, комплектности, объёму и качеству оклейки указанных элементов Заказчик <b>не имеет</b>.</p>' +
    '<p>5. Перечень оклеенных элементов (п.2) является окончательным и согласованным сторонами. Претензии о неоклейке либо неполной оклейке элементов, не заявленные Заказчиком при подписании настоящего акта, после его подписания <b>не принимаются</b>.</p>' +
    '<p>6. Автомобиль передан Заказчику. Настоящий акт составлен в двух экземплярах, по одному для каждой стороны, и является неотъемлемой частью договора № {{Номер договора}}.</p>' +
    '<div class="sign" style="margin-top:18px"><span>Исполнитель _____________ / {{Директор}}<br><span class="muted">(подпись)</span></span>' +
    '<span>Заказчик _____________ / ' + clientSign + '<br><span class="muted">(подпись — автомобиль и работы принял, претензий нет)</span></span></div>';
}
...
function buildDocPlaceholders_(d, serviceMode) {
  ...
  return {
    ...
    // Нумерованная таблица оклеенных элементов (для акта приёма — клиент подтверждает каждый элемент)
    'Таблица элементов оклейки': (function(){
      ...
      var rows = arr.map(function(e, i){ return '<tr><td class="center">' + (i + 1) + '</td><td>' + e + '</td><td class="center">оклеено&nbsp;✓</td></tr>'; }).join('');
      return '<table><thead><tr><th style="width:34px">№</th><th>Оклеенный элемент</th><th style="width:96px">Отметка</th></tr></thead><tbody>' + rows + '</tbody></table>';
    })(),
