...
function buildDocPlaceholders_(d) {
  ...
  return {
    ...
    'Предмет договора': (function(){
      ...
      if (/тонир|tint/i.test(o['Услуга'] || ''))
        return 'по тонировке а/м ' + avto + ', VIN номер ' + (o['VIN']||'') + ', плёнкой ' + film + (o['Светопропускаемость'] ? ' ' + o['Светопропускаемость'] + '%' : '');
      return 'по оклейке кузова а/м ' + avto + ', VIN номер ' + (o['VIN']||'') + ' защитной плёнкой ' + film;
    })(),
    'Акт работа': (function(){
      var avto = (marka + ' ' + model).trim();
      if (/тонир|tint/i.test(o['Услуга'] || ''))
        return '- тонировка а/м ' + avto + ', VIN номер ' + (o['VIN']||'') + ', плёнкой ' + film + (o['Светопропускаемость'] ? ' ' + o['Светопропускаемость'] + '%' : '') + ';';
      return '- оклейка кузова а/м ' + avto + ', VIN номер ' + (o['VIN']||'') + ' защитной плёнкой ' + film + ';';
    })(),
    'Протокол услуга': (/тонир|tint/i.test(o['Услуга'] || '') ? 'Тонировка автомобиля' : 'Оклейка кузова защитной плёнкой кузова автомобиля'),
