...
function addOrderDocColumns() {
  ...
  const need = ['Год выпуска', 'Пробег', 'Светопропускаемость', 'Элементы', 'Подписант', 'Доверенность', 'Паспорт', 'Дата начала работ', 'Дата окончания работ', 'Паспорт выдан', 'Адрес', 'Кол-во элементов'];
  const added = [];
